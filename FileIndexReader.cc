...
FileIndexReader::FileIndexReader(const string& file_name,
  ...
  if(fread(&header_, sizeof(IndexFileHeader), 1, file_) != 1) {
    //cerr << "Could not read file header" << endl;
    fclose(file_);
    Verify333(false);
  }
  ...
  if (validate) {
    ...
    while (left_to_read > 0) {
      ...
      int bytes_reading = std::min(kBufSize, left_to_read);

      int bytes_read = fread(buf, 1, bytes_reading, file_);

      if(bytes_read != bytes_reading) {
