...
void MemIndex_AddPostingList(MemIndex* index, char* word, DocID_t doc_id,
  ...
  if (word == NULL || strlen(word) == 0) {
    printf("Invalid or empty word. Skipping addition.\n");
    return;
  }
  ...
  printf("Current number of words in index: %d\n", MemIndex_NumWords(index));
  ...
  // as an argument.
  // if (HashTable_Find(wp->postings, doc_id, &postings_kv)) {
  //   LinkedList* existingPostings = postings_kv.value;
  // } else {
...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  if (!ret_list) {
        fprintf(stderr, "Failed to allocate memory for the result list.\n");
        return NULL;
  }
...
// Handling based on the number of query words
if (query_len == 1) {
...
int num = LinkedList_NumElements(ret_list);
  printf("size of re_list after: %d \n",num );
  ...
  for (i = 1; i < query_len; i++) {
    ...
    for (j = 0; j < num_docs; j++) {
      LLIterator_Get(ll_it, (void**) &ht_it);
      if(HashTable_Find(wp->postings, ht_it->doc_id, &kv)){
        ht_it->rank = ht_it->rank + LinkedList_NumElements(kv.value);
