...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // STEP 1
  char rootpathbuff[PATH_MAX], testfilebuffer[PATH_MAX];
  
  if (!realpath(root_dir.c_str(), rootpathbuff)) {
    return false;
  }
  ...
  size_t rootpath_len = strlen(rootpathbuff);
  size_t testfile_len = strlen(testfilebuffer);

  if (testfile_len < rootpath_len) {
    return false;
  }
  
   if (strncmp(rootpathbuff, testfilebuffer, rootpath_len) != 0) {
    return false;
  }

  // Ensure that the test file is within the root directory
  if (testfilebuffer[rootpath_len] != '/' && testfile_len != rootpath_len) {
    return false;
  }
